    ...
    return (
        <section class='preview' aria-label={`${model.section.courseName}课表预览`}>
            <div class='schedule' role='img' aria-label={`${model.section.courseName}每周课表`}>
                ...
                {days.map((day, index) => <div class='schedule__day' key={day} style={{ gridColumn: index + 2 }}>周{day}</div>)}
                {slots.map(slot => <div class='schedule__slot' key={slot} style={{ gridRow: slot + 1 }}>{slot}</div>)}
                <div class='schedule__canvas' />
                {model.blocks.map(block => <PreviewBlock block={block} key={block.id} />)}
            </div>
        </section>
    )
}
